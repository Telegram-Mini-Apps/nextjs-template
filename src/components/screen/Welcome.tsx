...
const WelcomeSlider: React.FC<WelcomeSliderProps> = ({ onSkip, onFinish }) => {
  const slides = [
    ...
    {
      image: "/img/outline-logo.png",
      title: "Ваш безопасный доступ в интернет с Outline",
      description: `Наш VPN-сервис построен на базе Outline, одного из самых надёжных и современных инструментов для обеспечения приватности в сети. Используя передовые технологии шифрования, Outline защищает ваши данные от посторонних глаз, позволяя вам беспрепятственно пользоваться интернетом.`,
    },
  ];
